...
- Text fallback: `{outcome: "review", data, message}`. Suggestions contain only supported name, DOB, age, gender, last-four and address fields; age is derived from a recognised date or year of birth ([ADR 0004](camp-operations/adr/0004-age-derived-from-date-of-birth.md)). Low-confidence words are omitted. Unknown fields remain absent. Raw OCR text, source photos and passwords are not returned or saved. Every suggestion requires review; it never creates or locks a person.
