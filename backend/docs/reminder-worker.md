...
Each HTTP request has a 120-second timeout. Transport errors, unsuccessful HTTP responses, invalid JSON, `ok: false` and an unconfigured SMS provider retry after 60, 120, 240 and then at most 300 seconds. A successful run resets the delay. Failed patient ledger rows can be retried by the endpoint; sent/pending rows are skipped. Restarting after 10:00 deliberately repeats the endpoint call, with duplicate suppression delegated to that ledger.
