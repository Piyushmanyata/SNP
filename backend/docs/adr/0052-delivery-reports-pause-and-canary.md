...
- **Submission outcome by phase.** A failure before the request reaches MSG91 (DNS, refused connection, TLS) is `failed` and is retried on a later run. A failure after the request was sent (timeout, reset, 5xx, unreadable reply) is `uncertain` and is never retried. MSG91's own error reply is `rejected`, with its reason, and is never retried.
