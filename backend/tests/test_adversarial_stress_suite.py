    ...
    def _matches(self, doc, query):
        for k, v in query.items():
            if k == "_id":
                ...
                if not any(self._matches(doc, part) for part in v):
                    return False
            elif k == "$expr":
                ...
                if isinstance(v, dict) and "$gt" in v:
                    ...
                    if doc.get(k) is None or not doc[k] < v["$lt"]:
                        return False
                elif "$lte" in v:
...
class TestFulfilmentStateMachineAndDeskStress:
    ...
    def test_correction_append_only_audit_log(self, monkeypatch):
        ...
        async def _run():
            ...
            rev_id = ObjectId()
            await mock_db.patients.insert_one({
                "_id": p_id, "camp_id": ObjectId(), "camp_day_id": ObjectId(), "queue_status": "seen", "full_name": "Corr Patient",
                "arrived_at": now_utc(), "printed_at": now_utc(), "seen_at": now_utc(),
                "committed_revision_id": rev_id, "clinical_generation": 1, "issue_auth_op": None,
            })
            await mock_db.prescription_revisions.insert_one({
            ...
            res = await add_correction(body, actor={"_id": ObjectId(), "role": "clinical_desk_operator"})
    ...
    def test_massive_concurrent_specs_deferral_race_50_workers(self, monkeypatch):
        ...
        async def _run():
            ...
            for i in range(50):
                ...
                await mock_db.prescription_revisions.insert_one({
                    "_id": rev_id, "patient_id": pid, "prescribed_lines": ["specs_made"],
                ...
                trans_ids.append((tid, rev_id))

            async def book_worker(tid_rev):
                tid, rev_id = tid_rev
                body = FulfilmentBody(
                    ...
                    paper_reviewed=True,
                    reviewed_revision_id=str(rev_id),
            ...
            fail_count = sum(1 for r in results if r.startswith("FAIL_"))

            assert ok_count == 50, f"Expected all 50 Specs assignments, got {ok_count} {results[:5]}"
            assert fail_count == 0

            day_doc = await mock_db.specs_collection_days.find_one({"_id": specs_day_id})
            assert day_doc.get("seats_taken") in (None, 0)
