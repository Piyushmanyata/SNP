...
from models import CorrectionBody, TranscriptionBody
...
def test_duplicate_inflight_issue_keeps_first_authorization(monkeypatch, paused_function):
    async def run():
        ...
        pending = asyncio.create_task(routes_clinical.record_fulfilment(body, actor=CLINICAL))
        await entered.wait()
        try:
            with pytest.raises(HTTPException) as exc:
                await routes_clinical.record_fulfilment(body, actor=CLINICAL)
            assert exc.value.status_code == 409
...
def test_late_issue_retry_preserves_the_newer_outcome(monkeypatch):
    async def run():
        ...
        original = await routes_clinical.record_fulfilment(first, actor=CLINICAL)
        latest = await routes_clinical.record_fulfilment(second, actor=CLINICAL)
        replayed = await routes_clinical.record_fulfilment(first, actor=CLINICAL)
...
def test_paused_pending_issue_rechecks_history_after_retry_and_newer_issue(monkeypatch):
    async def run():
        ...
        pending = asyncio.create_task(routes_clinical.record_fulfilment(first, actor=CLINICAL))
        await entered.wait()
        try:
            replayed = await routes_clinical.record_fulfilment(first, actor=CLINICAL)
            latest = await routes_clinical.record_fulfilment(second, actor=CLINICAL)
...
def test_issue_retry_finishes_failed_cleanup_without_releasing_ot_twice(monkeypatch, failure_stage):
    async def run():
        ...
        await routes_clinical.record_fulfilment(first, actor=CLINICAL)
        ...
        with pytest.raises(RuntimeError):
            await routes_clinical.record_fulfilment(second, actor=CLINICAL)
        current = await db.patients.find_one({"_id": patient["_id"]})
        ...
        result = await routes_clinical.record_fulfilment(second, actor=CLINICAL)
        ...
        assert await routes_clinical.record_fulfilment(second, actor=CLINICAL) == result
