...
- Configure the MSG91 account and approved DLT templates, then verify actual carrier delivery. Surgery texts include आधार कार्ड, वोटर आईडी और मोबाइल नंबर. A missing provider configuration intentionally skips delivery and causes the reminder worker to retry; it does not imply that SMS was sent.
...
531 automated tests pass: **356 backend tests**, including live HTTP/MongoDB workflows against the final rebuilt images, and **175 frontend tests across 25 suites**, including the prescription snapshot. No backend tests were skipped in the full live run. Anonymous HTTP clients are isolated per test so prior login cookies cannot invalidate unauthorized-access checks.
