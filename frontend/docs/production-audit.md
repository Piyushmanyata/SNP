...
| Clinical writes | Operations were not bound to patient/content, duplicate requests could release another issue's authorization, explicit correction clears were ignored, and delayed drafts could overwrite completed care | `backend/tests/test_clinical_operation_safety.py`; [clinical operation contract](../../backend/docs/clinical-operation-safety.md) |
