...
- **On a laptop the door opens with the USB box ready and focused.** It stays visible, clears after every scan and takes focus back, so the imager needs no button press per patient. A phone door opens on the camera button.
- **A burst never lands in the wrong field.** The door now also listens for a wedge burst wherever focus is, as registration already did (ADR 0023). Keys typed into the USB box are left to the box. Anywhere else, once a burst is recognised its keystrokes stop reaching the focused field, its terminator is swallowed, the field gets back the value it had before the burst, and the payload is read by the same scanner, with the same status line. Without this, a scan into the household-phone field kept its first ten digits and its Enter submitted the walk-in with a stranger's number.
- **A request that fails is not an unreadable card.** A network or server error while resolving a scan is shown, but not counted toward the failed attempts that open Manual entry.
...
- **The ambient listener as the only USB path at the door.** ADR 0034 rejected an invisible listener and the reason stands; the visible, focused box is the path, and the listener only rescues a burst that lands elsewhere, reporting it in the same scanner status.
