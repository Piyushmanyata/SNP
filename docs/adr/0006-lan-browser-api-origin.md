...
- Credentialed CORS allows only the explicit origins in `CORS_ORIGINS`. Local and LAN setups must name the actual frontend origin. RFC1918/loopback regex matching is retired.
- `cors_origin_list` drops `*` so credentialed cookies never pair with a wildcard origin.
