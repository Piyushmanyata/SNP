...
- **One scanner reads both codes.** `AadhaarScanner` takes an `onPatientCode` callback and matches `^snp:[a-z0-9-]+$` before decoding. A patient code goes to `/desk/lookup`; everything else goes to `/aadhaar/decode` as before. The prefix is ours and an Aadhaar Secure QR cannot collide with it. A **found** patient answers in the shape the live scanner recognises as a lock (`{outcome: "card"}`), because that is the only result that tears the camera down; a code that matches nothing answers `not-aadhaar`, so the loop keeps scanning and the volunteer can present another without restarting.
- **Identification is last-one-wins.** The door scan, the patient-code lookup and the name search all claim the same `findSequence` ref and drop their result if a newer request has started. They are three ways to ask one question — who is in front of me — and a slow answer to an abandoned question must never populate the row whose Print button stamps arrival.
